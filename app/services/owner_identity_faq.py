...
MSG_RAYA_MASCULINE = (
    "Raya Ameha Meresa is the creator of AethOS.\n\n"
    "He is building AethOS as a personal execution system that helps people think "
    "clearly and get things done."
)
MSG_RAYA_NEUTRAL = (
    "Raya Ameha Meresa is the creator of AethOS.\n\n"
    "Raya is building AethOS as a personal execution system that helps people think "
    "clearly and get things done."
)
...
_RE_WHO_CREATED_PRODUCT = re.compile(
    r"(?is)^\s*who\s+created\s+(nexa|aethos)\s*[?.!]*\s*$",
)
_RE_WHAT_IS_PRODUCT = re.compile(
    r"(?is)^\s*what\s+is\s+(nexa|aethos)\s*[?.!]*\s*$",
)
...
def try_canned_nexa_product_faq(user_message: str) -> str | None:
    ...
    if not t or len(t) > 200 or "\n" in t:
        return None
    if _RE_WHO_CREATED_PRODUCT.match(t):
        return "AethOS was created by **Raya Ameha Meresa**."
    if _RE_WHAT_IS_PRODUCT.match(t):
        return (
            "AethOS is an **AI execution system**: it helps you think, decide, and get work done — "
    ...
    if _RE_WHO_ARE_YOU.match(t):
        return (
            "I’m **AethOS** — the execution layer in this app. I answer plain questions directly and "
...
def try_canned_owner_identity_faq(
    ...
    *,
    ...
    if p in ("she", "her", "hers"):
        return (
            "Raya Ameha Meresa is the creator of AethOS.\n\n"
            "She is building AethOS as a personal execution system that helps people think "
            "clearly and get things done."
        )
