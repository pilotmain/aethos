...
**Verified 2026-05-14** against `Settings` (`app/core/config.py`), `app/services/intent_classifier.py`, `app/services/response_composer.py`, and `app/services/llm/completion.py` (`providers_available`, **`is_ollama_ready()`** in `app/services/llm/bootstrap.py`). **Setup:** `scripts/setup.py` autodetects the `ollama` CLI (`shutil.which`) during **Configure .env** and, when present, writes **`NEXA_OLLAMA_ENABLED=true`**, **`NEXA_LLM_PROVIDER=ollama`**, **`NEXA_PURE_LOCAL_LLM_MODE=true`**, and **`NEXA_OLLAMA_DEFAULT_MODEL`** when unset (recommended **`qwen2.5:7b`**). Step 6 asks for Anthropic / OpenAI / DeepSeek keys as **optional fallbacks** only; the wizard prints an **LLM configuration summary** (primary vs cloud vs heuristics-only). **Recommended Ollama model tags and RAM notes:** see **§4.3**.
...
| Fresh **`scripts/setup.py`** and **`ollama`** on `PATH` | Target `.env` gets **`NEXA_OLLAMA_ENABLED=true`**, **`NEXA_LLM_PROVIDER=ollama`**, **`NEXA_PURE_LOCAL_LLM_MODE=true`**, and default model tag when unset (**CLI presence**; you still need a running Ollama server and a pulled model for successful local inference). |
