...
cd nexa-mobile
npm install
npx react-native init NexaMobileTmp --version 0.73.11 --skip-install
# Copy generated android/ and ios/ from NexaMobileTmp into nexa-mobile/, then merge package.json scripts if needed.
rm -rf NexaMobileTmp
```
...
Point the app at your Nexa API (defaults to `http://localhost:8000`). For a physical device, use your machine's LAN IP.
...
Connects to `ws(s)://<host>/api/v1/mobile/ws/chat?token=<jwt>`. The server currently echoes JSON; replace with Nexa gateway streaming when wiring production chat.
