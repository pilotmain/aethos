...
def test_intent_rule_create_agent() -> None:
    assert get_intent("create an agent for legal") == "create_custom_agent"
    assert get_intent("Create agent @foo") == "create_custom_agent"
    assert get_intent("  create agent x") == "create_custom_agent"
