...
import type { PermissionRequiredPayload } from "@/lib/aethos-types";
...
type WebAethosWorkspaceProjectRow = {
  id: number;
  name: string;
  path_normalized: string;
...
/** Composer hints — Phase 26: no legacy AethOS slash list; click fills the composer. */
const SLASH_HINTS: { cmd: string; help: string; fill: string }[] = [
  ...
  { cmd: "show memory", help: "what AethOS remembers", fill: "show memory" },
...
function agentLabel(ak: string | null | undefined) {
  ...
  if (k === "nexa") return "AethOS";
  /** Legacy router personas collapse to AethOS in labels (no separate persona chips). */
  if (
...
function JobRowMini({ job, onPick }: { job: AethosJob; onPick: (id: number) => void }) {
...
function UnconfiguredPrompt() {
  return (
    <div className="flex min-h-[calc(100vh-0px)] w-full max-w-[1920px] flex-1 flex-col items-center justify-center gap-4 bg-zinc-950 p-8 text-center text-sm text-zinc-300">
      <p className="text-zinc-100">Connect AethOS</p>
...
function WorkspaceBody() {
  ...
  const [aethosWsProjects, setAethosWsProjects] = useState<WebAethosWorkspaceProjectRow[] | null>(null);
  const [aethosWsBusyId, setAethosWsBusyId] = useState<number | null>(null);
  ...
  const [jobPanelList, setJobPanelList] = useState<AethosJob[] | null>(null);
  ...
  const [sendingActivity, setSendingActivity] = useState("AethOS is thinking…");
  ...
  const [jobById, setJobById] = useState<Record<number, AethosJob>>({});
  ...
  const mergeJob = useCallback((j: AethosJob) => {
    setJobById((prev) => ({ ...prev, [j.id]: { ...prev[j.id], ...j } }));
  }, []);

  const pollWebJobUntilTerminal = useCallback(
    async (jobId: number): Promise<AethosJob | null> => {
      const maxAttempts = 50;
      for (let a = 0; a < maxAttempts; a++) {
        const j = await webFetch<AethosJob>(`/web/jobs/${jobId}`);
        mergeJob(j);
    ...
    async (permId: string, mode: "once" | "session") => {
      ...
      try {
        ...
        void (async () => {
          ...
          if (!head) {
            try {
              head = await webFetch<AethosJob>(`/web/jobs/${hostJid}`);
              mergeJob(head);
            } catch {
              setMessages((prev) => [
                ...prev,
          ...
          const terminal =
            ...
            st0 === "error" ||
            st0 === "blocked";
          const fmtResult = (j: AethosJob): string => {
            const st = (j.status || "").toLowerCase();
            if (st === "completed") {
              return (
                (j.result || "").trim() ||
  ...
  const loadAccessPanel = useCallback(async () => {
    ...
    try {
      const [perms, roots, nxProj] = await Promise.all([
        ...
        webFetch<WebAethosWorkspaceProjectRow[]>(`/web/workspace/nexa-projects`),
      ]);
      setAccessPerms(perms);
      setAccessRoots(roots);
      setAethosWsProjects(nxProj);
    } catch (e) {
  ...
  const activateAethosWorkspaceProject = useCallback(
    async (projectId: number | null) => {
      if (!isConfigured()) {
        return;
      }
      setAethosWsBusyId(projectId ?? -1);
      setAccessPanelErr(null);
      try {
  ...
  const loadInitialData = useCallback(async () => {
    ...
    try {
      ...
      const [raw, jobs] = await Promise.all([
        ...
        webFetch<AethosJob[]>(`/web/jobs?limit=20`),
      ]);
      setMessages(
        raw.map((m) => ({
          id: id(),
  ...
  useEffect(() => {
    ...
    const run = () => {
      for (const jid of activeJobsToPoll) {
        void webFetch<AethosJob>(`/web/jobs/${jid}`).then(mergeJob).catch(() => {
  ...
  useEffect(() => {
    ...
    void webFetch<AethosJob[]>(`/web/jobs?limit=50`)
      .then((rows) => {
  ...
  const allJobsForPanel = useMemo(() => {
    const m = new Map<number, AethosJob>();
  ...
  const send = async () => {
    ...
    if (tl.startsWith("run dev")) {
      ...
      setSendingActivity("AethOS is thinking…");
    }
    try {
  ...
  return (
    <div className="flex h-[calc(100vh-0px)] min-h-[480px] w-full min-w-0 max-w-[1920px] items-stretch">
      <aside className="flex w-16 shrink-0 flex-col border-r border-white/10 bg-black/30 py-3">
        <div className="px-1 text-center text-[9px] font-bold uppercase tracking-tighter text-emerald-200/80">AethOS</div>
              ...
              {headerSession ? (
                <>
                  <p className="truncate font-medium text-zinc-100">
                    {headerSession.title || headerSession.active_topic || "AethOS"}
                  </p>
                  <p className="text-[10px] text-zinc-500">Messages · {headerSession.message_count}</p>
                </>
              ) : (
                <p className="text-zinc-100">AethOS</p>
              )}
            </div>
        ...
        {isConfigured() && showReleaseBanner && releaseLatest && (
          <div className="shrink-0 border-b border-zinc-800 bg-zinc-900 px-6 py-2 text-sm text-zinc-300 xl:px-10">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div className="min-w-0 flex-1">
                <p className="font-medium text-zinc-100">What’s new in AethOS</p>
        ...
        <div ref={scrollRef} className="min-h-0 flex-1 overflow-y-auto overflow-x-hidden py-3 md:py-4">
          ...
          {messages.length === 0 && !sending && !dataError && (
            <div className="mx-auto max-w-md py-8 text-center text-sm text-zinc-400">
              <p className="text-lg text-zinc-100">Welcome to AethOS</p>
              ...
              <ul className="mt-2 list-none space-y-1.5 pl-0 text-left text-sm text-zinc-400">
                ...
                <li>
                  ...
                  — what AethOS allowed or blocked
                </li>
                    ...
                    <div className="mt-2 rounded-xl border border-amber-500/25 bg-amber-500/[0.06] px-3 py-2.5">
                      ...
                      <p className="mt-1.5 text-[10px] uppercase tracking-wide text-zinc-500">
                        {(m.permissionRequired.scope || "").toLowerCase() === "file_write"
                          ? "AethOS wants to write:"
                          : "AethOS wants to read:"}
                      </p>
              ...
              {(m.decision || m.web_tool_line || (m.sources && m.sources[0]) || m.usage_subline) && (
                <details className="mt-1.5 text-[10px] text-zinc-500">
                  <summary className="cursor-pointer list-none pl-0 marker:content-[''] [&::-webkit-details-marker]:hidden text-zinc-500 hover:text-zinc-300">
                    <span className="underline decoration-dotted decoration-zinc-600/50">What AethOS used</span>
                  </summary>
          ...
          <div className="min-h-0 flex-1 overflow-y-auto overflow-x-hidden p-3 text-sm">
            {rightTab === "job" && (
              <div className="text-zinc-300">
                ...
                {focusJobId && jobById[focusJobId] ? (
                  ...
                  <p className="mt-1 text-xs text-zinc-500">Select a job below or ask AethOS in chat.</p>
                )}
                ...
                {jobPanelList !== null && allJobsForPanel.length === 0 && (
                  <p className="mt-2 text-sm text-zinc-500">No active jobs yet. Ask AethOS to create one.</p>
                )}
            ...
            {rightTab === "system" && (
              <div>
                ...
                <div className="mb-3 rounded border border-white/10 bg-white/[0.02] p-2.5">
                  ...
                  <p className="mt-1 text-[10px] text-zinc-500">
                    Named folders under your roots — AethOS uses the active one as the default path for file intel in
                    this chat (no scanning). Telegram: /projects, /project use &lt;id&gt;.
                  </p>
                  {aethosWsProjects && aethosWsProjects.length === 0 && (
                  ...
                  {aethosWsProjects && aethosWsProjects.length > 0 && (
                    <ul className="mt-2 space-y-1.5">
                      {aethosWsProjects.slice(0, 12).map((p) => (
                        <li
                          key={p.id}
                        ...
                        >
                          ...
                          <button
                            type="button"
                            disabled={aethosWsBusyId !== null}
                            onClick={() => void activateAethosWorkspaceProject(p.id)}
                  ...
                  {aethosWsProjects && aethosWsProjects.length > 0 && (
                    <button
                      type="button"
                      disabled={aethosWsBusyId !== null}
                      onClick={() => void activateAethosWorkspaceProject(null)}
              ...
              <div className="text-xs text-zinc-300">
                ...
                {keys && (
                  <p className="text-zinc-500">
                    You can use AethOS with your own API key. Add OpenAI or Anthropic here. Keys are stored server-side, never in chat.
                  </p>
                )}
                {keys && (
            ...
            {rightTab === "usage" && (
              <div className="space-y-3 text-xs text-zinc-300">
                <div className="flex items-center justify-between gap-1">
                  <h3 className="text-xs font-semibold text-zinc-200">AethOS usage (today)</h3>
                  <button
                    type="button"
                ...
                <div className="text-[9px] leading-relaxed text-zinc-500">
                  <p className="text-zinc-400">Why AethOS is efficient</p>
              ...
              <div className="text-xs text-zinc-300">
                ...
                {!docLoad && docList && docList.length === 0 && !docErr && (
                  <p className="mt-3 text-sm text-zinc-500">
                    No documents yet. Export an assistant response or ask AethOS to create a PDF.
                  </p>
                )}
