...
/** NexaForge soft launch — read-only catalog + safe preview flags (Phase 55). */
export function NexaForgePanel(props: { shellLight: boolean }) {
    ...
    <section className={`rounded-xl border p-4 ${border} ${bg}`}>
      <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          ...
          <h2 className={`text-sm font-semibold ${title}`}>NexaForge (preview)</h2>
        </div>
        <button
          type="button"
          onClick={() => void load()}
      ...
      <p className={`mb-3 text-[11px] ${muted}`}>
        Catalog ships as <code className="rounded bg-black/10 px-1">data/nexa_marketplace/catalog.json</code>.
